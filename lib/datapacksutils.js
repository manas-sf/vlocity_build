...
VLOCITY_BUILD_SALESFORCE_API_VERSION = '51.0';
...
DataPacksUtils.prototype.getListFileName = function(dataPackType, SObjectType, isUpgradeObjects) {
  
    var listFileNameKeys = this.getExpandedDefinition(dataPackType, SObjectType, "ListFileName", isUpgradeObjects);
...
DataPacksUtils.prototype.getFileName = function(dataPackType, SObjectType, isUpgradeObjects) {
    return this.getExpandedDefinition(dataPackType, SObjectType, "FileName", isUpgradeObjects);
}
...
DataPacksUtils.prototype.getFolderName = function(dataPackType, SObjectType, isUpgradeObjects) {
    return this.getExpandedDefinition(dataPackType, SObjectType, "FolderName", isUpgradeObjects);
}

DataPacksUtils.prototype.getFileType = function(dataPackType, SObjectType, isUpgradeObjects) {
    return this.getExpandedDefinition(dataPackType, SObjectType, "FileType", isUpgradeObjects);
}
...
DataPacksUtils.prototype.getVisualForcePagetemplate = function(dataPackType, urlType) {
    return this.getExpandedDefinition(dataPackType, null, urlType);
}
...
DataPacksUtils.prototype.isDeployLast = function (dataPackType) {
    return this.getExpandedDefinition(dataPackType, null, "DeployLast");
}
...
DataPacksUtils.prototype.getExpandedDefinition = function(dataPackType, SObjectType, dataKey, isUpgradeObjects) {
    let definitionValue;
    if (isUpgradeObjects && !this.vlocity.namespace) {
...
DataPacksUtils.prototype.getAllParentChildKeys = async function(dataPacks, jobInfo) {
    ...
    for (var dataPack of dataPacks) {
        var newParents = [];
        if (
            dataPack.VlocityDataPackType === 'Project' &&
            dataPack.VlocityDataPackData && dataPack.VlocityDataPackData['%vlocity_namespace%__Project__c'] &&
            dataPack.VlocityDataPackData['%vlocity_namespace%__Project__c'][0].RecordTypeId.DeveloperName === 'Module'
        ) {
            let projectItems = dataPack.VlocityDataPackData['%vlocity_namespace%__Project__c'][0]['%vlocity_namespace%__ProjectItem__c'];
            if (projectItems) {
                projectItems.forEach(projectItem => {
                    if (typeof (projectItem['%vlocity_namespace%__ItemId__c']) === 'string') {
                        if (jobInfo.dataPackKeyToPrimarySourceKey[projectItem['%vlocity_namespace%__ItemId__c']]) {
                            newParents.push(jobInfo.dataPackKeyToPrimarySourceKey[projectItem['%vlocity_namespace%__ItemId__c']]);
                        }
                    } else {
                        if (jobInfo.dataPackKeyToPrimarySourceKey[projectItem['%vlocity_namespace%__ItemId__c'].VlocityLookupRecordSourceKey]) {
                            newParents.push(jobInfo.dataPackKeyToPrimarySourceKey[projectItem['%vlocity_namespace%__ItemId__c'].VlocityLookupRecordSourceKey]);
                        }
                    }
                });
            }
    ...
    for (var filePath of filePaths) {
        ...
        for (var apexChunk of apexChunks) {
            try {
                ...
                throw `Apex Error >> ${err.exceptionStackTrace || err.compileProblem || err.exceptionMessage || 'APEX code failed to execute but no exception message was provided'}`;
            }
        }
    }
};
        ...
        if (sourceDataPack.hasOwnProperty('SFDXData')) {
            ...
            if (jobInfo.diffType[dataPackKey] == 'Changed') {
                ...
                for (var change of allFieldDiffsByRecordSourceKey) {
                    if (change.diffType == 'Changed' || change.diffType == 'New' || change.diffType == 'Deleted') {
                        jobInfo.diffType[dataPackKey] = 'Changed';
                        break;
                    }
                }
...
DataPacksUtils.prototype.getDiffsForSfdx = function(jobInfo, diffType, sourceSfdxObject, targetSfdxObject) {
    ...
    if (type == 'utf8') {
        ...
        if (ignoredDiffTypes.includes(type)) {
...
DataPacksUtils.prototype.getAdditionalSFMetadataDependencies = async function(jobInfo, sfFolderPath){
    let vlocityDataPackTypes = [],
        salesforceKeys = [],
        salesforceTypes = [];

    vlocityDataPackTypes = Object.keys(await this.vlocity.utilityservice.getVlocityDataPackConfigurations());
    
    salesforceKeys = jobInfo.specificManifestKeys.filter(item => {
        return vlocityDataPackTypes.indexOf(item.split('/')[0]) === -1;
    });

    salesforceTypes = Array.from(new Set(
        salesforceKeys.map(item => {
            return item.split('/')[0];
        })
    ));

    let allMetaDataPrefixMappings = {},
        allMetaDataSuffixMappings = {},
        selectedMetadataMappings = {};
    this.allMetadata.metadataObjects.forEach(metaDesc => {
        allMetaDataPrefixMappings[metaDesc.xmlName] = metaDesc.directoryName;
        allMetaDataSuffixMappings[metaDesc.xmlName] = metaDesc.suffix || '';
        if (salesforceTypes.indexOf(metaDesc.directoryName) !== -1) {
            if(metaDesc.xmlName !== 'ApexClass'){
                selectedMetadataMappings[metaDesc.xmlName] = metaDesc.directoryName;
            }
        }
    });

    VlocityUtils.report('Retrieving Salesforce Metadata Dependencies');

    let dependenciesArr = [];
    for (let key of Object.keys(selectedMetadataMappings)) {
        try {
            let query = `Select MetadataComponentName, MetadataComponentType, RefMetadataComponentName, RefMetadataComponentType from MetadataComponentDependency WHERE RefMetadataComponentType = '${key}'`;
            VlocityUtils.report('Query', query);
            let foundMetadataComponents = await this.vlocity.jsForceConnection.tooling.query(query);
            VlocityUtils.report('Found Records', foundMetadataComponents.records.length);
            dependenciesArr.push(foundMetadataComponents);
        } catch (err) {
            VlocityUtils.error('Query Error', err);
        }
    }

    dependenciesArr = dependenciesArr.concat(await this.retrieveExceptionalMetadataDependencies(jobInfo, salesforceTypes));
    
    let allComponentMetadataDependencies = [];
    dependenciesArr.forEach(item => {
        allComponentMetadataDependencies = allComponentMetadataDependencies.concat(item.records);
    });

    let metadataComponentMappings = {};
    allComponentMetadataDependencies.forEach(item => {
        let keyPrefix = allMetaDataPrefixMappings[item.RefMetadataComponentType];
        let keySuffix = allMetaDataSuffixMappings[item.RefMetadataComponentType];

        let valuePrefix = allMetaDataPrefixMappings[item.MetadataComponentType];
        let valueSuffix = allMetaDataSuffixMappings[item.MetadataComponentType];

        if (keyPrefix != undefined && keySuffix != undefined && valuePrefix != undefined && valueSuffix != undefined) {
            let key = keyPrefix + '/' + item.RefMetadataComponentName + (keySuffix ? '.' + keySuffix : '');
            let value = valuePrefix + '/' + item.MetadataComponentName + (valueSuffix ? '.' + valueSuffix : '');
            
            if (!metadataComponentMappings[key]) {
                metadataComponentMappings[key] = [];
            }
            metadataComponentMappings[key].push(value);
        }
    });

    fs.outputFileSync(path.join(sfFolderPath, 'SalesforceReferences.yaml'), yaml.safeDump(metadataComponentMappings), "UTF-8");

    let newSFKeysToBeAddedToManifest = [];
    salesforceKeys.forEach(item => {
        if (metadataComponentMappings[item]) {
            metadataComponentMappings[item].forEach(component => {
                if (!salesforceKeys.includes(component)) {
                    newSFKeysToBeAddedToManifest.push(component);
                }
            })
        }
    });

    return newSFKeysToBeAddedToManifest;
}

DataPacksUtils.prototype.retrieveExceptionalMetadataDependencies = async function(jobInfo, salesforceTypes) {
    var exceptionDetailsObj = {},
        dependenciesArr = [];

    if (salesforceTypes.indexOf('classes') !== -1) {
        exceptionDetailsObj['ApexClass'] = {
            allDataQuery: 'SELECT Id,Name FROM ApexClass WHERE NamespacePrefix = null',
            prefix: 'classes',
            suffix: '.cls',
            mappingKeyParam: 'Name',
            mappingValueParam: 'Id',
            metadataComponentFilterParam: 'RefMetadataComponentId'
        }
    }
        
    for (const key of Object.keys(exceptionDetailsObj)) {
        var allData = await this.vlocity.utilityservice.fetchAllRecords(exceptionDetailsObj[key].allDataQuery);
        var selectedManifestMappings = {};
        allData.forEach(item => {
            let mappingKey = exceptionDetailsObj[key].prefix + '/' + item[exceptionDetailsObj[key].mappingKeyParam] + exceptionDetailsObj[key].suffix;
            if (jobInfo.specificManifestKeys.indexOf(mappingKey) !== -1) {
                selectedManifestMappings[mappingKey] = item[exceptionDetailsObj[key].mappingValueParam];
            }
        });
        for (const dataPackKey of Object.keys(selectedManifestMappings)) {
            try {
                let query = `Select MetadataComponentName, MetadataComponentType, RefMetadataComponentName, RefMetadataComponentType from MetadataComponentDependency WHERE ${exceptionDetailsObj[key].metadataComponentFilterParam} = '${selectedManifestMappings[dataPackKey]}'`;
                VlocityUtils.report('Query', query);
                let foundMetadataComponents = await this.vlocity.jsForceConnection.tooling.query(query);
                VlocityUtils.report('Found Records', foundMetadataComponents.records.length);
                dependenciesArr.push(foundMetadataComponents);
...
DataPacksUtils.prototype.retrieveSalesforce = async function(jobInfo, listOnly) {
    ...
    if (jobInfo.specificManifestKeys) {
        ...
        if(jobInfo.maxDepth !== 0){
            let additionalSFDependencies = await this.getAdditionalSFMetadataDependencies(jobInfo, sfdxProject.sfdxPath);
            jobInfo.specificManifestKeys = jobInfo.specificManifestKeys.concat(additionalSFDependencies);

            VlocityUtils.report('Found Salesforce Metadata Dependencies', additionalSFDependencies);
        }
...
DataPacksUtils.prototype.readProfileAndWriteAsXML = async function(profileName, writePath) {
    ...
    try {
        if (read.fullName) {
            ...
            if (writePath.indexOf('-meta.xml') == -1) {
                writePath = `${writePath}-meta.xml`;
            }
            VlocityUtils.log('Writing Profile', writePath);
            fs.outputFileSync(writePath, xmlbuilder.create(xmlFormatted, { encoding: 'UTF-8' }).end({ pretty: true, indent: '    '}));
        }
    } catch (e) {
    ...
    if (profiles.length > 0 || permissionSets.length > 0) {
        ...
        for (var profile of profiles) {
            try {
                ...
                let profileName = profile.substring(profile.lastIndexOf('/')+1, profile.lastIndexOf('.profile')); 
                let profileInOrg = await this.vlocity.jsForceConnection.metadata.read('Profile', [ profileName ]);
                ...
                currentProfile.Profile.layoutAssignments = this.filterInvalidLayouts(currentProfile.Profile.layoutAssignments, layoutNames, currentProfile.Profile.recordTypeVisibilities);

                currentProfile.Profile.tabVisibilities = this.filterInvalidTabs(currentProfile.Profile.tabVisibilities, profileInOrg, allCustomObjMetadata);
        ...
        for (var permissionSet of permissionSets) {
            ...
            for (var permissionSetNode of profileNodes) {
                ...
                if (currentPermissionSet.PermissionSet[permissionSetNode.type] && currentPermissionSet.PermissionSet[permissionSetNode.type].length == 0) {
            ...
            if (currentPermissionSet.PermissionSet.tabSettings) {
                currentPermissionSet.PermissionSet.tabSettings = this.filterInvalidTabs(currentPermissionSet.PermissionSet.tabSettings, null, allCustomObjMetadata);
            }
...
DataPacksUtils.prototype.filterInvalidTabs = function(currentArray, existingProfile, allSObjects) {

    let processedArray = [];
    // Ignore this for Permission Sets - Should be null for Permission Set and Empty Object for a profile that does not exist.
    let existingTabs = existingProfile ? {} : null ;

    if (existingProfile && existingProfile.tabVisibilities) {
        existingProfile.tabVisibilities.forEach(tab => {
            existingTabs[tab.tab] = tab.visibility;
        });
    }
        ...
        for (var item of currentArray) {
            ...
            if (stnd == 0) {
                ...
                if (allSObjects[objectType] 
                    && (!existingTabs 
                        || (existingTabs[item.tab[0]] 
                        && existingTabs[item.tab[0]] != item.visibility[0]))) {
                    processedArray.push(item);
                } else {
...
DataPacksUtils.prototype.filterInvalidLayouts = function(currentArray, layoutNames, recordTypes) {
    ...
    let validRecordTypes = recordTypes.map(recType => {
        if (recType && recType.recordType) {
            return recType.recordType[0];
        }
    });
    ...
    if (currentArray) {
        for (var item of currentArray) {
            ...
            if (layoutNames.includes(decodeURIComponent(layoutName)) && (!item.recordType || validRecordTypes.includes(item.recordType[0]))) {
                processedArray.push(item);
            } else {
...
DataPacksUtils.prototype.parallelRetrieve = async function(input) {
    ...
    try {
        ...
        if (jobInfo.maxDepth !== 0) {
            var retrievedLWCDependencies = await this.retrieveLwcDependencies(response.inboundFiles, input, jobInfo);
            response.inboundFiles = response.inboundFiles.concat(retrievedLWCDependencies.inboundFiles);
            jobInfo.specificManifestKeys = Array.from(new Set(jobInfo.specificManifestKeys.concat(retrievedLWCDependencies.retrievedLWCDependencyKeys)));
        }
...
DataPacksUtils.prototype.retrieveLwcDependencies = async function (retrievedMetadata, input, jobInfo) {
    let allRetrievedLwcHtmlFiles = [];
    retrievedMetadata.forEach(item => {
        if (item.filePath) {
            if (item.filePath.includes('lwc') && item.filePath.includes('.html')) {
                allRetrievedLwcHtmlFiles.push(item.filePath);
            } 
        } else if (item.type == 'LightningComponentBundle' && item.fullName.includes('.html')) {
            allRetrievedLwcHtmlFiles.push(path.join(jobInfo.sfdxExpandFolder, 'DX', 'salesforce_sfdx', 'main','default','lwc', item.fullName));
        }
    });
    ...
    let deployments = {
        targetusername: input.deployments.targetusername
    };

    var retrievedLWCDependencyKeys = [],
        inboundFiles = [];

    htmlFileContent.forEach(item => {
        let htmlContentArr = item.split('<');
        htmlContentArr.forEach(content => {
            if (content.indexOf('c-') === 0) {
                let lwcKey = (content.split('>')[0]).split(' ')[0].split('\n')[0].split('-')[1];
                let manifestKey = 'lwc/' + lwcKey;
                if (!jobInfo.specificManifestKeys.includes(manifestKey)) {
                    if (deployments.metadata) {
                        deployments.metadata += ',';
                    } else {
                        deployments.metadata = '';
                    }
                    deployments.metadata += `${this.metadataTypesByFolder['lwc'].xmlName}:${lwcKey}`;
                    retrievedLWCDependencyKeys.push(manifestKey);
                }
            }
        });
    });
    ...
    if (deployments.metadata) {
        deployments.metadata = Array.from(new Set(deployments.metadata.split(','))).join(',');
        var response = await this.vlocity.utilityservice.sfdx('source:retrieve', deployments);
        input.deployments = deployments;
        inboundFiles = response.inboundFiles;
        let selfReturnResponse = await this.retrieveLwcDependencies(response.inboundFiles, input, jobInfo);
        retrievedLWCDependencyKeys = retrievedLWCDependencyKeys.concat(selfReturnResponse.retrievedLWCDependencyKeys);
        inboundFiles = inboundFiles.concat(selfReturnResponse.inboundFiles);
    } 

    return {
        retrievedLWCDependencyKeys: Array.from(new Set(retrievedLWCDependencyKeys)),
...
DataPacksUtils.prototype.getSFDXProject = function(jobInfo) {
    ...
    if (!fs.existsSync('sfdx-project.json') || jobInfo.deleteSfdxProject || jobInfo.resetSfdxProjectLocation) { 
        ...
        let sfAPIVersion = VLOCITY_BUILD_SALESFORCE_API_VERSION;
        let potentialLocalFile = path.join(jobInfo.sourceProjectPath, 'sfdx-project.json');

        if ((jobInfo.source == 'local' || jobInfo.target == 'local') && fs.existsSync(potentialLocalFile)) {
            let sfdxFile;
            try {
                sfdxFile = JSON.parse(fs.readFileSync(potentialLocalFile));
                if (sfdxFile) {
                    sfAPIVersion = sfdxFile.sourceApiVersion;
                }
            } catch (e) {
...
DataPacksUtils.prototype.getSfdxData = async function(jobInfo) {
    ...
    let sfdxReferences = {};
    if (fs.existsSync(path.join(sfdxProject.sfdxPath, 'SalesforceReferences.yaml'))) {
        sfdxReferences = yaml.safeLoad(fs.readFileSync(path.join(sfdxProject.sfdxPath, 'SalesforceReferences.yaml'), 'utf8'));
    }
        ...
        for (let file of this.getFiles(path.join(sfdxProject.sfdxPath, sfdxFolder, dir))) {
            ...
            if (file[0] != '.') {
                ...
                try {
                    ...
                    let sfDataPackReferences = sfdxReferences[dataPackKey];
                    var fileData = {
                        ...
                        VlocityDataPackParents: sfDataPackReferences ? sfDataPackReferences : []
                    };
...
DataPacksUtils.getAsManifestInfo = function(dataPack) {
    let dataPackKeySplit = dataPack.VlocityDataPackKey.replace('-meta.xml', '').split('/');

    let type = dataPackKeySplit.shift();
    let label = dataPackKeySplit.join('/');

    let keyForManifest = dataPack.VlocityDataPackKey.replace('-meta.xml', '');

    // Layouts can have "/"
    if (folderOnlyMetadata.includes(type) && type != "layouts") {
        keyForManifest = `${type}/${dataPackKeySplit[0]}`;
        label = dataPackKeySplit[0];
    }

    if (label.indexOf(`${type}/`) == 0) {
        label = label.substr(`${type}/`.length);
    }

    return { key: keyForManifest, label: label };
}

DataPacksUtils.prototype.getAsManifestInfo = function(dataPack) {
    return DataPacksUtils.getAsManifestInfo(dataPack);
}
...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        ...
        try {
            ...
            if (fs.existsSync(tempSfdxFolder)) {
                VlocityUtils.verbose('Preparing Profiles and Permission Sets for Deployment');
                ...
                VlocityUtils.log('Deploying Profiles and Permission Sets Separately');
...
DataPacksUtils.prototype.updateDataPackWithNewMappings = function (dataPack, jobInfo) {
    let obj = {};
    let dataPackType = dataPack.VlocityDataPackType;

    let dataField = this.getDataField(dataPack);
    let mappings = this.vlocity.omnistudio.platformObjectMappings[dataPackType][dataField];

    if (mappings) {
        
        this.handleDataPackEventSync('addParentReference', dataPackType, dataPack);

        let newDataField = mappings.objectName;
        obj[newDataField] = dataPack.VlocityDataPackData[dataField];
        ...
        Object.keys(obj[newDataField][0]).forEach(key => {
            ...
            if (Object.keys(this.vlocity.omnistudio.platformObjectMappings[dataPackType]).includes(key)) {
                let subMappings = this.vlocity.omnistudio.platformObjectMappings[dataPackType][key];
                let childObjDataField = subMappings.objectName;
                obj[newDataField][0][childObjDataField] = [];

                if (!obj[newDataField][0][key]) {
                    obj[newDataField][0][key] = [];
                }
                // Iterate through array of Level 2 Objects
                // Level 2 object like DRMapItems__c in DRBundle__c & Element__c in OmniScript__c
                obj[newDataField][0][key].forEach(subObj => {
                    let level2Obj = subObj;
                    let parentElementKeyName = '';
                    Object.keys(subObj).forEach(fieldName => {
                        if (subMappings.fieldNames[fieldName]) {
                            if(subObj[fieldName][parentElementKeyName]){
                                subObj[fieldName][subMappings.fieldNames[parentElementKeyName]] = JSON.parse(JSON.stringify(subObj[fieldName][parentElementKeyName]).replace(new RegExp(key, 'g'), childObjDataField).replace(new RegExp(dataField, 'g'), newDataField));
                                delete subObj[fieldName][parentElementKeyName];
                            }
                            if (subObj[fieldName].VlocityMatchingRecordSourceKey) {
                               // subObj[fieldName].VlocityMatchingRecordSourceKey = subObj[fieldName].VlocityMatchingRecordSourceKey.replace(new RegExp(key, 'g'), childObjDataField).replace(new RegExp(dataField, 'g'), newDataField);
                                subObj[fieldName].VlocityRecordSObjectType = subObj[fieldName].VlocityRecordSObjectType.replace(new RegExp(key, 'g'), childObjDataField).replace(new RegExp(dataField, 'g'), newDataField);
                                parentElementKeyName = fieldName;
                            }
                            level2Obj[subMappings.fieldNames[fieldName]] = subObj[fieldName];
                            if (fieldName !== subMappings.fieldNames[fieldName]) {
                                delete level2Obj[fieldName];
                            }
                        }
                    });
                    level2Obj.VlocityRecordSObjectType = childObjDataField;
                  //  level2Obj.VlocityRecordSourceKey = level2Obj.VlocityRecordSourceKey.replace(new RegExp(key, 'g'), childObjDataField).replace(new RegExp(dataField, 'g'), newDataField);
                    obj[newDataField][0][childObjDataField].push(level2Obj);
                });
                delete obj[newDataField][0][key];
            }
        });
