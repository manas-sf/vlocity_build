...
var SUPPORTED_JOB_KEY_TO_OPTION_MAP = {
    ...
    forceOldDataModel: 'forceOldDataModel'
};
...
DataPacksJob.prototype.initializeJobInfo = async function(jobInfo, action) {
    ...
    jobInfo.upgradeDataPackFields = jobInfo.upgradeDataPackFields || false;
...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    } else if (action === 'convertToCore') {
        await this.convertToCore(jobInfo);
    } else if (action === 'validateParentKeys') {
        await this.validateParentKeys(jobInfo);
    } else if (action === 'checkOrgType') {
        jobInfo.data = {
            isOmniStudioInstalled: this.vlocity.isOmniStudioInstalled,
            isOmniStudioIndustryInstalled: this.vlocity.isOmniStudioIndustryInstalled
        }
    } else {
...
DataPacksJob.prototype.getAllParentKeysIntoManifest = async function(jobInfo) {

    let jobInfoTemp = JSON.parse(JSON.stringify(jobInfo));
    jobInfoTemp.specificManifestKeys = null;
    let importPath = jobInfo.projectPath + '/' + jobInfo.expansionPath;
    
    await this.vlocity.datapacksbuilder.initializeImportStatus(importPath, jobInfoTemp);
    jobInfo.keysToDirectories[dataPackKey]
    let totalCurrentParents;
    do {
        totalCurrentParents = jobInfo.specificManifestKeys.length;

        for (var dataPackKey of jobInfo.specificManifestKeys) {
            var fullPathToFiles = path.join(importPath, jobInfoTemp.keysToDirectories[dataPackKey]);
            var dataPackLabel = this.vlocity.datapacksbuilder.getDataPackLabelByDir(fullPathToFiles, jobInfoTemp);
            let parentData = this.vlocity.datapacksbuilder.getFileData(fullPathToFiles, dataPackLabel + '_ParentKeys.json');
            
            if (parentData) {
                for (let parentKey of JSON.parse(parentData)) {
                    if (jobInfo.specificManifestKeys.indexOf(parentKey) == -1) {
                        jobInfo.specificManifestKeys.push(parentKey);
                    }
                }
            }
        }
    } while (totalCurrentParents != jobInfo.specificManifestKeys.length);
}
...
DataPacksJob.prototype.formatManifest = async function(jobInfo) {
    ...
    if (jobInfo.gitCheck && jobInfo.jobAction == 'Deploy') {
        ...
        if (jobInfo.inlcudeDependencies) {
            await this.getAllParentKeysIntoManifest(jobInfo);
        }
...
DataPacksJob.prototype.addRecordToExport = function(jobInfo, queryData, record) {
    ...
    if (jobInfo.fullManifest[queryData.VlocityDataPackType][record.Id] || 
        (jobInfo.specificManifestKeys && queryData.manifestOnly && jobInfo.specificManifestKeys.indexOf(record.VlocityDataPackKeyForManifest) == -1 && jobInfo.specificManifestKeys.indexOf(record.VlocityDataPackKeyForManifest) == -1) ||
        (jobInfo.specificManifestKeys 
    ...
    record.orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl(record.VlocityDataPackType, record);
...
DataPacksJob.prototype.runQueryForManifest = async function(queryInput) {
    let promise = await new Promise((resolve) => {
        ...
        VlocityUtils.verbose("Using SF API Version", this.vlocity.jsForceConnection.version);
            ...
            if (dataPack.VlocityDataPackStatus == 'Success') {
                ...
                if (!jobInfo.maxDepth || jobInfo.maxDepth == -1 || dataPack.VlocityDepthFromPrimary == 0) {
                    ...
                    if (dataField && dataPack.VlocityDataPackData && dataPack.VlocityDataPackData[dataField]) {
                        ...
                        let orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl(dataPack.VlocityDataPackType, dataPack.VlocityDataPackData[dataField][0]);
...
DataPacksJob.prototype.buildFile = async function(jobInfo) {
    ...
    if (jobInfo.buildFile) {
        ...
        if (dataJson) {
            ...
            dataJson.dataPacks.forEach(dataPack => {
                delete dataPack.VlocityDataPackChildren;
                let dataField = this.vlocity.datapacksutils.getDataField(dataPack);
                if (dataField) {
                    delete dataPack.VlocityDataPackData[dataField][0].sourceOrgUrl;
                }
            });

            if (fileName) {
                fs.outputFileSync(fileName, stringify(dataJson), 'utf8');
            }
...
DataPacksJob.prototype.expandFile = async function (jobInfo) {
    if (this.vlocity.isOmniStudioInstalled) {
        jobInfo.upgradeDataPackFields = true;
    }
...
DataPacksJob.prototype.updateSettings = async function(jobInfo) {
    ...
    jobInfo.upgradeDataPackFields = false;
...
DataPacksJob.prototype.activateAll = async function(dataPack, jobInfo, attempts) {
    ...
    let allActivated = activateResult.allActivated || [];
    let allActivationErrors = activateResult.allActivationErrors || {};
...
DataPacksJob.prototype.deployPack = async function(inputMap) {
    ...
    try {
        ...
        for(let dataPack of dataJson.dataPacks){
            await this.vlocity.datapacksutils.handleDataPackEvent('preDeployDataPackCheck', dataPack.VlocityDataPackType, { dataPack: dataPack, jobInfo: jobInfo });
        }
            ...
            for (var dataPack of dataPackData.dataPacks) {
                ...
                if (dataPack.VlocityDataPackStatus == 'Success') {
                    ...
                    if (jobInfo.headersOnly) {
                        ...
                        if(jobInfo.versionCompare){
                            ...
                            if (dataField && dataPack.VlocityDataPackData && dataPack.VlocityDataPackData[dataField]) {
                                ...
                                jobInfo.VlocityDataPackKeyToUrlMapping[dataPack.VlocityDataPackKey] = {
                                    targetOrgUrl: this.vlocity.utilityservice.getVisualForcePageUrl(dataPack.VlocityDataPackType, dataPack.VlocityDataPackData[dataField][0], datapackId)
                                }
                            }
                        }
                    }
                    ...
                    if (dataPack.VlocityDataPackMessage && dataPack.VlocityDataPackMessage.indexOf('unable to obtain') != -1) {
                        ...
                        var errorMessageText = dataPack.VlocityDataPackMessage;

                        if (!errorMessageText) {
                            if (dataPackData.errors 
                            && dataPackData.errors[dataPack.VlocityDataPackKey] 
                            && dataPackData.errors[dataPack.VlocityDataPackKey][0]) {
                                errorMessageText = dataPackData.errors[dataPack.VlocityDataPackKey][0].VlocityDataPackMessage;
                            }
                        }

                        if (!errorMessageText) {
                            errorMessageText = 'No error message from server';
                        }
        ...
        VlocityUtils.error('Error', 'Deploying Pack', dataPack.VlocityDataPackKey);
...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if (!jobInfo.isRetry) {        
        ...
        if (jobInfo.autoFixPicklists && !this.vlocity.isOmniStudioInstalled) {
    ...
    if (jobInfo.hasError && jobInfo.jobAction === 'Deploy' && jobInfo.autoRetryErrors) {
        ...
        if (jobInfo.previousErrors === undefined || currentErrors < jobInfo.previousErrors) {
            ...
            await this.vlocity.datapacksutils.saveCurrentJobInfo(jobInfo, true);
            await this.initializeJobInfo(jobInfo, 'Retry');
            await this.doRunJob(jobInfo, jobInfo.jobAction);
        }
    }
...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        var field_name = this.vlocity.namespacePrefix +'Data__c';
        var timestampFilter = '';
        if(lastTimeStamp){
            timestampFilter = `And ${this.vlocity.namespacePrefix}Timestamp__c >= ${lastTimeStamp}`
        }
        var query = `SELECT ${field_name},CreatedBy.Username,CreatedBy.Profile.Name FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action', 'OS Step Unload') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ${timestampFilter} ORDER BY CreatedDate DESC LIMIT 50000`;
        ...
        try {
            var totalEntries = await this.vlocity.jsForceConnection.query(`SELECT count() FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action', 'OS Step Unload') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ${timestampFilter}`);
        } catch (err) {
            VlocityUtils.verbose('Process Profiler data is not available for this User');
            jobInfo.hasError = true;
            resolve();
        }
        ...
        if (totalEntries.totalSize == 0) {
            ...
            totalEntries.totalSize = 50000;
        }

        var i = existingRecords.length;
        ...
        this.vlocity.jsForceConnection.query(query)
            .on("record", function (record) {
                i++;
                if ((i % 500) == 0) {
                    VlocityUtils.log(`${i} of ${totalEntries.totalSize + existingRecords.length}`);
                }

                if (!lastTimeStamp || (lastTimeStamp && new Date(JSON.parse(record[field_name]).Timestamp) > new Date(lastTimeStamp))) {
                    records.push(record);
                }
            })
            .on("end", async function () {
                ...
                for (var i = 0; i < records.length; i++) {
                    ...
                    let data = JSON.parse(item[field_name]);
                    ...
                    Object.keys(data).forEach(key => {
                        ...
                        if (dataKey) {
                            ...
                            data[dataKey] = data[key];
                        }
                    });
                
                    omni_id.push(data.OmniScriptId);

                    item[field_name] = JSON.stringify(data);
                }

                VlocityUtils.log(`${existingRecords.length + i} of ${totalEntries.totalSize + existingRecords.length}`);
                ...
                while (unique_omni_id.length > 0) {
                    try{
                        ...
                        if (unique_omni_id_format) {
                            ...
                            let additional_data = await self.vlocity.jsForceConnection.query(self.vlocity.omnistudio.updateQuery(`SELECT ${self.vlocity.namespace}__PropertySet__c,Name,${self.vlocity.namespace}__Type__c,${self.vlocity.namespace}__OmniScriptId__c 
                            ...
                            additional_data.records.map(item => {
                                if(self.vlocity.isOmniStudioInstalled){
                                    elements[item['Name'] + item.OmniProcessId] = item;
                                } else {
                ...
                records.forEach(item => {
                    try {
                        ...
                        if (!element_data.ElapsedTime) {
                            return;
                        }
                        
                        if (!element_data.VlocityInteractionToken) {
                            if (element_data.InstanceIdentifier) {
                                element_data.VlocityInteractionToken = element_data.InstanceIdentifier;
                            } else {
                                element_data.VlocityInteractionToken = element_data.Timestamp;
                            }
                        }

                        element_data.Name = 'StepActionTime';
                        element_data.TrackingEvent = 'StepActionTime';
 
                        element_data.Username = item.CreatedBy.Username;
                        element_data.UserProfile = item.CreatedBy && item.CreatedBy.Profile && item.CreatedBy.Profile.Name;

                        if (elements[key]) {                        
                            let propertyset = JSON.parse(elements[key][self.vlocity.namespace + '__PropertySet__c'] || elements[key].PropertySetConfig);
                            var element_type = elements[key][self.vlocity.namespace + '__Type__c'] || elements[key].Type;
                ...
                if (existingRecords.length > 0) {
                    res = res.concat(existingRecords);
                }
          
                if (res.length > 0) {
                    try {
                        await fs.outputFile(jobInfo.projectPath, JSON.stringify(res));
                        jobInfo.hasError = false;
                    } catch (err) {
                        jobInfo.hasError = true;
                        jobInfo.errors.push(err);
...
DataPacksJob.prototype.getOrgProjects = async function (jobInfo) {
    try {
        let query = `SELECT Name, RecordType.Name FROM ${this.vlocity.namespacePrefix}Project__c WHERE ${this.vlocity.namespacePrefix}IsActive__c = true AND ((${this.vlocity.namespacePrefix}Status__c IN ('Deployed', 'Released') AND RecordType.DeveloperName = 'WorkSet') OR RecordType.DeveloperName = 'Module')`;
        let result = await this.vlocity.jsForceConnection.query(query);
...
DataPacksJob.prototype.getTrackingEntryUrl = async function (jobInfo) {
    try {
        ...
        if (actionsConfig[entry.ElementType]) {
            ...
            let dataFieldValue = '';
            config.dataField.forEach(fieldName => {
                if (!dataFieldValue) {
                    dataFieldValue = fieldName.split('.').reduce(function (currentNameObj, key) {
                        return currentNameObj ? currentNameObj[key] : '';
                    }, entry);
                }
            });
            ...
            if (dataFieldValue) {
                ...
                let results = await this.vlocity.jsForceConnection.query(this.vlocity.omnistudio.updateQuery(query));
                if (results.records[0] && results.records[0].Id) {
                    orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl(config.type, results.records[0]);
                }
            }
        }
        if (!actionsConfig[entry.ElementType] || !orgUrl) {
            let obj = {
                'Id': entry.OmniScriptId,
                '%vlocity_namespace%__IsLwcEnabled__c': true
            }
            if (entry.TrackingService === 'OmniScript') {
                orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl('OmniScript', obj);
            } else {
                orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl('IntegrationProcedure', obj);
            }
        }
        jobInfo.data = orgUrl;
    } catch (err) {
    ...
    try {
        for (const obj of jobInfo.queries) {
            if (obj.VlocityDataPackType === jobInfo.type) {
                ...
                let additionalCondition = `${this.vlocity.namespace}__Type__c = '${jobInfo.omniType}' AND ${this.vlocity.namespace}__SubType__c = '${jobInfo.omniSubType}'`
                query += query.includes('WHERE') ? ` AND ${additionalCondition}` : ` Where ${additionalCondition} `;
                let results = await this.vlocity.jsForceConnection.query(this.vlocity.omnistudio.updateQuery(query));
                if (results.records[0] && results.records[0].Id) {
                    jobInfo.data = this.vlocity.utilityservice.getVisualForcePageUrl(jobInfo.type, results.records[0]);
                } else {
                    throw (`DataPack does not exist for type - ${jobInfo.omniType}, subtype - ${jobInfo.omniSubType}.`)
                }
                break;
            }
        }
...
DataPacksJob.prototype.convertToCore = async function (jobInfo) {
    if (jobInfo.outputDir) {
        jobInfo.upgradeDataPackFields = true;
        jobInfo.singleFile = true;
        let dataJSON = await this.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo);
        this.vlocity.datapacksexpand.expand(jobInfo.outputDir, dataJSON, jobInfo);
    } else {
        jobInfo.hasError = true;
        jobInfo.errors.push(`'outputDir' parameter not specified`);
        VlocityUtils.error(`'outputDir' parameter not specified`);
    }
}
...
DataPacksJob.prototype.validateParentKeys = async function (jobInfo) {
    ...
    for (var dataPack of dataJSON.dataPacks) {
        ...
        this.vlocity.datapacksutils.getParents(jobInfo, dataPack, dataPack.VlocityDataPackKey);
    }

    for (var dataPack of dataJSON.dataPacks) {
        let dataPackData = dataPack.VlocityDataPackData[this.vlocity.datapacksutils.getDataField(dataPack)][0];
        let dataPackName = this.vlocity.datapacksexpand.getDataPackName(dataPack.VlocityDataPackType, dataPackData.VlocityRecordSObjectType, dataPackData);
        let localFileKey = dataPack.VlocityDataPackType + "/" + dataPackName;
        let allParentKeys = dataPack.VlocityDataPackParents;
        dataPack.VlocityDataPackParents.forEach((parentKey) => {
        ...
        await this.vlocity.datapacksutils.handleDataPackEvent('onGenerateParentKeys', dataPack.VlocityDataPackType, { dataPackData: dataPack, allParentKeys: allParentKeys });

        var self = this;
        allParentKeys.forEach(function (parentKey) {
            parentKey = parentKey.replace(/%vlocity_namespace%__/g, '').replace(/__c/g, '');
            if (!parentKey || finalParentKeys.indexOf(parentKey) != -1) {
                return;
            }

            if (localFileKey != parentKey
                && dataPack.VlocityDataPackKey != parentKey
                && parentKey.indexOf('|Page|') == -1
                && !self.vlocity.datapacksutils.isGuaranteedParentKey(parentKey)
                && Object.keys(jobInfo.keysToDirectories).includes(parentKey)) {
        ...
        if (finalParentKeys.length > 0) {
            ...
            await this.vlocity.datapacksexpand.writeFile(dataPack.VlocityDataPackType, dataPack.VlocityDataPackName, dataPackName + "_ParentKeys", "json", finalParentKeys);
        }
    }
}